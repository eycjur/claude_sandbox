...
[Docker Sandboxes](https://docs.docker.com/ai/sandboxes/)（`sbx`）上で AI コーディングエージェント（Claude Code など）を使い捨てサンドボックスとして起動する CLI です。
...
- サンドボックスは microVM で分離されており、サンドボックスの IP へホストから直接は届きません。Web サーバーを動かす場合は `0.0.0.0` で listen し、`agentsb open <port>` でポートを公開してから `http://localhost:<port>/` でアクセスしてください。
